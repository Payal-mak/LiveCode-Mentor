...
2. One high-quality article — for C++ code prefer popular Medium articles or GeeksforGeeks; for Python prefer official docs or Real Python
...
- If it's a prime/math check → recommend Number Theory or Math tagged problems
...
Return ONLY valid JSON. No markdown."""
    ...
    try:
        ...
        # 2-attempt parse — same pattern as other endpoints
        try:
...
async def get_trace(payload: CodePayload):
    ...
    if payload.language != "python":
        # C++/Java cannot be traced — return a clear, correctly-shaped response
        return {
            "steps":       [],
            "output":      "",
            "success":     False,
...
Return ONLY valid JSON. No markdown. No explanation."""
    ...
    # ── Call Groq ─────────────────────────────────────────────────────────────
    # Mock inputs used by trace_code for any input() calls in Python
    mock_inputs = ["5", "1 2 3 4 5", "3", "1 2 3", "10", "hello", "0"]
    ...
    try:
        ...
        mock_inputs = []   # auto-test doesn't need mock inputs — code runs directly
...
async def get_current_score():
    score = get_score()
    newly_earned = check_and_award_badges(score)   # returns list of newly earned badge IDs
    earned = get_badges()
